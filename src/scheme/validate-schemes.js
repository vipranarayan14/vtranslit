...
export const validateSchemes = (schemes) => {
  ...
  if (!schemes) {
    throw new Error(
      'Function vTranslit should be initiated with a list vTranslit schemes.'
    );
  }
  ...
  // handle single scheme array
    ...
    throw new Error(
      'Atleast two vTranslit schemes are required to init vTranslit.'
    );
  } else {
    schemes.map((scheme) => {
      if (
        ...
        throw new Error(
          'One or more of the provided vTranslit scheme are invalid.'
        );
      }
    });
